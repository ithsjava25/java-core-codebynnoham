...
class WarehouseAnalyzer {
        ...
        for (Map.Entry<Category, List<Product>> e : byCat.entrySet()) {
            ...
            for (Product p : items) {
                 /* if (p instanceof Shippable s) {
                    ...
                    if (w.compareTo(BigDecimal.ZERO) > 0) {  //if (w > 0)
                        BigDecimal wBD = w;
                        //weightedSum = weightedSum.add(p.getPrice().multiply(wBD));
                        weightedSum = weightedSum.add(p.getPrice().multiply(wBD));
                        weightSum.add(wBD);   //weightSum += w;
                    }
                }  */
                if (p instanceof Shippable shippable) {
                    BigDecimal price = p.getPrice();
                    BigDecimal weight = Optional.ofNullable(shippable.weight()).orElse(BigDecimal.ZERO);
            ...
            BigDecimal avg;
            if (weightSum.compareTo(BigDecimal.ZERO) > 0) {  // if (weightSum > 0)
    ...
    public List<Product> findPriceOutliers(double standardDeviations) {
        ...
        double sum = products.stream().map(Product::getPrice).mapToDouble(bd -> bd.doubleValue()).sum();
        double mean = sum / n;
        ...
        for (Product p : products) {
            double diff = Math.abs(p.getPrice().doubleValue() - mean);
    ...
    public List<ShippingGroup> optimizeShippingGroups(BigDecimal maxWeightPerGroup) {
        ...
        List<Shippable> items = new ArrayList<>(warehouse.getProducts().stream()
        ...
        items.sort((a, b) -> Objects.requireNonNullElse(b.weight(), BigDecimal.ZERO)
        ...
        for (Shippable item : items) {
            ...
            for (List<Shippable> bin : bins) {
                ...
                BigDecimal binWeight = bin.stream()
                        ...
                        .reduce(BigDecimal.ZERO, (a, b) -> a.add(b));
                if (binWeight.add(w).compareTo(maxWeightPerGroup)<= 0) { //binWeight + w <= maxW
                    bin.add(item);
                    placed = true;
                    break;
                }
    ...
    public Map<Product, BigDecimal> calculateExpirationBasedDiscounts() {
        ...
        for (Product p : warehouse.getProducts()) {
            ...
            if (p instanceof Perishable per) {
                ...
                if (daysBetween == 0) {
                    discounted = p.getPrice().multiply(new BigDecimal("0.50"));
                } else if (daysBetween == 1) {
                    discounted = p.getPrice().multiply(new BigDecimal("0.70"));
                } else if (daysBetween > 1 && daysBetween <= 3) {
                    discounted = p.getPrice().multiply(new BigDecimal("0.85"));
                } else {
                    discounted = p.getPrice();
                }
    ...
    public InventoryValidation validateInventoryConstraints() {
        ...
        long highValueCount = items.stream().filter(p -> p.getPrice().compareTo(highValueThreshold) >= 0).count();
        double percentage = (highValueCount * 100.0) / items.size();
        int diversity = (int) items.stream().map(Product::getCategory).distinct().count();
    ...
    public InventoryStatistics getInventoryStatistics() {
        ...
        BigDecimal totalValue = items.stream().map(Product::getPrice).reduce(BigDecimal.ZERO, BigDecimal::add);
        ...
        int categoryCount = (int) items.stream().map(Product::getCategory).distinct().count();
        Product mostExpensive = items.stream().max(Comparator.comparing(Product::getPrice)).orElse(null);
        Product cheapest = items.stream().min(Comparator.comparing(Product::getPrice)).orElse(null);
